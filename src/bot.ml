...
let make_shutdown send () =
  Lwt_main.at_exit (fun () ->
      Lwt_unix.with_timeout 1.0 (fun () -> code_of_close Final |> Websocket.Frame.close |> send));
  raise Exit
...
module Make (Bot : S) : sig
  ...
  let close_connection { ic; oc; send; cancel = _ } close =
    ...
    let%lwt () =
      ...
      then Lwt.return_unit
        ...
        Lwt.catch
          (fun () -> Lwt_unix.with_timeout 1.0 (fun () -> send @@ Websocket.Frame.close code))
  ...
  let connect (Login.{ token; _ } as login) state =
