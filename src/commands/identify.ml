...
let%expect_test "Intent bitfield to int" =
  ...
  test (Intents.of_list [ GUILD_MEMBERS; GUILD_MESSAGES ]);
  [%expect {| 514 |}];
  test (Intents.of_list [ GUILD_MEMBERS; GUILD_MESSAGES; GUILD_MEMBERS ]);
  [%expect {| 514 |}];
  test (Intents.of_list [ GUILD_MEMBERS; GUILD_MESSAGES; GUILD_VOICE_STATES ]);
  [%expect {| 642 |}]
