...
let with_connection ?(extra_headers = Cohttp.Header.init ()) ?(random_string = Websocket.Rng.init ())
   ?(ctx = Conduit_lwt_unix.default_ctx) client uri =
  let connect () =
    ...
    let drain_handshake () =
      ...
      >>= fun () ->
      fail_unless (status = `Switching_protocols) (fun () -> protocol_error "wrong status") >>= fun () ->
      begin
      ...
      >>= fun () ->
      fail_unless (upgrade_present headers) (fun () -> protocol_error "upgrade header not present")
      >>= fun () ->
      begin
  ...
  let write_frame frame =
    Buffer.clear buf;
    Lwt.wrap2 (write_frame_to_buf ~mode:(Client random_string)) buf frame >>= fun () ->
...
let write_failed_response oc =
  ...
  let response = Cohttp.Response.make ~status:`Forbidden ~encoding:(Cohttp.Transfer.Fixed body_len) () in
  let open Response in
  write ~flush:true (fun writer -> write_body writer body) response oc

let establish_server ?read_buf ?write_buf ?timeout ?stop
   ?(on_exn = (fun exn -> !Lwt.async_exception_hook exn)) ?(check_request = check_origin_with_host)
  ...
  let server_fun flow ic oc =
    (Request.read ic >>= function
     ...
     | `Eof ->
       ...
       Lwt_log.info ~section "Remote endpoint closed connection" >>= fun () -> Lwt.fail End_of_file
     | `Invalid reason ->
    ...
    begin
      match
        version, meth, C.Header.get headers "upgrade", key, upgrade_present headers, check_request request
      with
      | `HTTP_1_1, `GET, Some up, Some key, true, true when String.Ascii.lowercase up = "websocket" ->
        Lwt.return key
      | _ -> write_failed_response oc >>= fun () -> Lwt.fail (Protocol_error "Bad headers")
    end
    >>= fun key ->
    let hash = key ^ websocket_uuid |> b64_encoded_sha1sum in
    let response_headers =
      C.Header.of_list [ "Upgrade", "websocket"; "Connection", "Upgrade"; "Sec-WebSocket-Accept", hash ]
    in
    let response =
      C.Response.make ~status:`Switching_protocols ~encoding:C.Transfer.Unknown ~headers:response_headers
        ()
    in
...
let establish_standard_server ?read_buf ?write_buf ?timeout ?stop ?on_exn ?check_request
   ?(ctx = Conduit_lwt_unix.default_ctx) ~mode react =
  let f client = react (Connected_client.make_standard client) in
  establish_server ?read_buf ?write_buf ?timeout ?stop ?on_exn ?check_request ~ctx ~mode f
